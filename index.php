<?php
    ...
    if (isset($_SERVER["HTTP_REFERER"])) {
        ...
        if (!in_array($referer_host, $dominios_autorizados)) {
...
<?php
    if ($ip === true) {
        crear_editar_log(
            $archivo,
            "El archivo " . __FILE__ . " ha sido cargado",
            0,
            $_SERVER['REMOTE_ADDR'],
            (array_key_exists("HTTP_REFERER", $_SERVER) ? $_SERVER["HTTP_REFERER"] : "NULL REFERER"),
...
<?php
        ...
        crear_editar_log(
            $archivo,
            "Dirección IP no autorizada ha cargado el archivo " . __FILE__ . " ha sido redirigido a: $redirect",
            1,
            $_SERVER['REMOTE_ADDR'],
